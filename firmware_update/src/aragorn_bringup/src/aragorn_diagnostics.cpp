...
class Diagnostic
{
    protected:
        ...
        int imu_recive_stamp_,
                      lds_recive_stamp_ = 0,
                      io_recive_stamp_= 0,
                      batt_recive_stamp_= 0,
                      motor_recive_stamp_= 0,
                      cam_recive_stamp_= 0,
                      cam2_recive_stamp_= 0;
        
        int imu_recive_stamp_old_,
                      lds_recive_stamp_old_= 0,
                      io_recive_stamp_old_= 0,
                      batt_recive_stamp_old_= 0,
                      motor_recive_stamp_old_= 0,
                      cam_recive_stamp_old_= 0,
                      cam2_recive_stamp_old_= 0;
        ...
        enum CHARGING_STATE charging = CHARGING_STATE::NORMAL;
        ...
        int TIMEOUT_NOACTION_BEFORE_SHUTDOWN = 10800; //sec
    ...
    public:
        Diagnostic()
        {
            ...
            ros::param::param<int>("~timeout_noaction_before_shutdown", TIMEOUT_NOACTION_BEFORE_SHUTDOWN, 10800);
            ...
            sub_robot_status = nh_.subscribe("/aragorn/base_status", 1, &Diagnostic::cbStatus, this);
            ...
            crsv_shutdown = nh_.serviceClient<std_srvs::SetBool>("/aragorn_io/shutdown");
        ...
        void step_diagnostics()
        {
            switch(current_diagnostic_step)
            {
                case DIAGNOSTIC_SEQUENCE::INIT_HARDWARE_TYPE:
                    ...
                    prev_robot_mode.data = current_robot_mode.data;
        ...
        void cbRobotMode(const std_msgs::Int8 &msg)
        {
            ...
            // if((current_robot_mode.data == robot_mode::RESET) || (current_robot_mode.data == robot_mode::CHARGER_OFF))
            // {
            //     if(battery_state.percentage >= 0.95)
            //     {
            //         ROS_INFO("robot_mode == fullcharge");
            //         _isFull_charge = true;
            //     }
            // }
        ...
        void diagnostic_automatics_shutdown_()
        {
            if(battery_state.power_supply_status != sensor_msgs::BatteryState::POWER_SUPPLY_STATUS_CHARGING)
            {
                if(current_robot_mode.data == robot_mode::READY_TO_START)
                {
                    if(charging == CHARGING_STATE::START_CHARGE)
                    {
                        if(battery_state.percentage >= 0.95000)
                        {
                            _isFull_charge = true;
                            charging = CHARGING_STATE::NORMAL;
                        }
                    }
                }


                if(current_robot_mode.data == prev_robot_mode.data)
                {
                    if(!robot_status_.robot_moving)
                    {
                        ...
                        if(!_init_saving)
                        {
                            if(_isFull_charge)
                            {
                                ROS_INFO("[aragorn_diagnostics]:set timeout shutdown by full charge");
                                saving_begin = ros::Time::now().toSec() + TIMEOUT_FULL_CHARGE_SHUTDOWN;
                                _isFull_charge = false;
                            }
                            else
                            {
                                saving_begin = ros::Time::now().toSec() + TIMEOUT_NOACTION_BEFORE_SHUTDOWN;
                            }
                        ...
                        {
                            // double timmer_ = saving_begin - ros::Time::now().toSec();
                            // ROS_INFO("[aragorn_diagnostics]:countdown saving power time %d ", timmer_);
                            if((saving_begin - ros::Time::now().toSec()) < 0)
                            {
                                //do something
                                std_srvs::SetBool shutdown_srv;
                                shutdown_srv.request.data = true;
                                if(crsv_shutdown.call(shutdown_srv))
                                {
                                    ;
                                }
                                ROS_INFO("[aragorn_diagnostics]:shutdown robot pc!!!");
                                _init_saving = false;
                            }
                            else;
                        }
                ...
                {
                    ROS_INFO("[aragorn_diagnostics]:robot_mode change useractive!!");
                    prev_robot_mode.data = current_robot_mode.data;
                    _init_saving = false;
                    //_isFull_charge = false;
                }
            ...
            }else
            {
                // ROS_INFO("[aragorn_diagnostics]:robot moving");
                _init_saving = false;
                charging = CHARGING_STATE::START_CHARGE;
            }
        }
        ...
        void diagnostic_acc_uvc()
        {
            if(uvc_state.data == UVC_STATE::UVC_LAMP_ON)
            {
                ...
                {
                    ROS_INFO("%ld", ros::Time::now().toSec() - acc_time_stamp_);
                }
            }
            else;
            {
        ...
        void system_ready_pub()
        {
            ...
            if((_isIO && _isIMU && _isCam && _isCam2 && _isBattery) == true)
