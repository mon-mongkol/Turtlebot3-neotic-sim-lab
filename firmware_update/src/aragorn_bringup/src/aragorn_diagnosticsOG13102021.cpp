...
class Diagnostic
{
    protected:
        ...
        std_msgs::Int8 robot_mode, interlock_state;
    ...
    public:
        Diagnostic()
        {
            ...
            sub_uvc_interlock = nh_.subscribe("/aragorn_io/interlock_state", 10, &Diagnostic::cbUVCInterlock, this);
            sub_robot_mode = nh_.subscribe("/aragorn/robot_mode", 10, &Diagnostic::cbRobotMode, this);

            sc_reset = nh_.serviceClient<std_srvs::SetBool>("/aragorn_io/reset");
        ...
        void diagnostic_charger_state()
        {
            ...
            if(system_status.data == true)
            {
                ...
                {
                    if((current_robot_mode != robot_mode::EMERGENCY) && (current_robot_mode != robot_mode::CHARGER_ON) && (current_robot_mode != robot_mode::READY_TO_START))
                    {
                    ...
                    {
                        if(current_robot_mode == robot_mode::CHARGER_ON)
                        {
                ...
                ROS_INFO("current robot mode %d", current_robot_mode);
            }
            else
            {
                emer_charge_msg.data = false;
