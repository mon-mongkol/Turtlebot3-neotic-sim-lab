...
public:
                                           ...
                                           move_base_ac("move_base", true)
    {
        ...
        ROS_INFO("serial %s %d", serial_no, serial_no.find("SMR0100"));
    ...
    void init_param_()
    {
        ...
        ros::param::param<std::string>("~command_xy", command_xy, "XY");
        ...
        ROS_INFO("serial %s %d", serial_no.c_str(), serial_no.find("SMR0100"));
    }
    ...
    void executeCB(const matrix_msgs::XbeeCommunicationGoalConstPtr &goal)
    {
        ...
        while (!task_completed)
        {
            ...
            case Task_Sequence::CmdSelector:
            {
                ...
                else if (data_info_.cmd_string == command_xy)
                {
                    ROS_INFO("[Matrix_XbeeMovementServer]:Go to State --> Direct MoveBase");
                    data_info_.cmd_uint = matrix_msgs::XbeeCommuROHM::CMD_XY;
                    feedback_.sequence = Task_Sequence::MoveBase_NavGoal;
                }
                else
                {
            ...
            case Task_Sequence::MoveBase_NavGoal:
            {
                ...
                if(data_info_.cmd_string == command_xy)
                {
                    mb_goal = goal->goal;
            ...
            case Task_Sequence::MOVE_TARGET_FAIL:
                ...
                // if (movefail_counter < movefail_repeats)
                if ((movefail_counter < movefail_repeats) && (data_info_.cmd_string != command_xy))
                {
                    ROS_ERROR("[Matrix_XbeeMovementServer]:current movefial_counter: %d", movefail_counter);
    ...
    {
        // SMR0100R always use DWA for Navigation forward and backward don't need to change to TEB
        if((serial_no.find("SMR0100")  != -1) && (serial_no.find("SMR0100R")  == -1))
        {
