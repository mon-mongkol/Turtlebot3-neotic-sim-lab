#ifndef _MATRIX_DOCKING_AR_OPERATION_NODE_H
#define _MATRIX_DOCKING_AR_OPERATION_NODE_H
...
#include <matrix_msgs/ARDockingOperationAction.h> 
...
class ARDockingOperation
{
    protected:
        ros::NodeHandle nh_;
        actionlib::SimpleActionServer<matrix_msgs::ARDockingOperationAction> as_; // NodeHandle instance must be created before this line. Otherwise strange error occurs
        std::string action_name_;
        // create messages that are used to published feedback/result
        matrix_msgs::ARDockingOperationFeedback feedback_;
        matrix_msgs::ARDockingOperationResult   results_;
        ...
        ros::ServiceClient docking_mode_sc, set_ios_sc, launch_controller_ac, enabled_ar_detect;
        ...
        std::string CHARGE_REQ="CHARGE_REQ";
        std::string ROBOT_CHARGING="ROBOT_IN_CHARGING_STATE";
        std::string CHARGE_FINISH="CHARGE_FINISH";
        std::string CG_REQ_SUCCESS="CG_REQ_SUCCESS";
    ...
    public:
...

        {
            ...
            enabled_ar_detect = nh_.serviceClient<std_srvs::SetBool>("enable_detections");
            ...
            // // //wait for the matrix_artrack_operation action server to come up
            while(!artrack3_operation_ac.waitForServer(ros::Duration(5.0))){
                ROS_INFO("Waiting for the matrix_artrack_operation action server to come up");
            }
            ...
            ROS_INFO("[Matrix_docking_ar_operation]: Matrix_ar_docking_operation action server is ready!!!!!");
    ...
    private:
        ...
        bool executeCB(const matrix_msgs::ARDockingOperationGoalConstPtr &goal);
        void preemptCB();
        ...
        bool setDetect_AR(std::string cmd);
