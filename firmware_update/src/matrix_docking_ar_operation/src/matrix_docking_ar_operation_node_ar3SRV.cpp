#include "matrix_docking_ar_operation/matrix_docking_ar_operation_node_ar3SRV.h"

using namespace matrix_msgs;

bool ARDockingOperation::executeCB(const matrix_msgs::ARDockingOperationGoalConstPtr &goal)
{
    ros::Rate r(10);
    ...
    while(!finished)
    {
        ...
        switch(feedback_.sequence)
        {
            case ARDockingControlState::INIT_PARM:
                ROS_INFO("[Matrix_docking_ar_operation]: INIT_PARM");
                feedback_.text = "INIT_PARM";
                
                //clear parameters before execute goal
                clear_params(false, false);
                ROS_INFO("[Matrix_docking_ar_operation]: INIT_PARM--->Cleara param successs");


                // LaunchController(ActionControllerRequest::CMD_ACTION_START, ActionControllerRequest::AC_AR3_REAR);
                ROS_INFO("[Matrix_docking_ar_operation]: INIT_PARM--->Launch AR3 successs");
                feedback_.sequence = ARDockingControlState::CMD_SELECTOR;
                
                break;

            case ARDockingControlState::CMD_SELECTOR:
                ROS_INFO("[Matrix_docking_ar_operation]: CMD_SELECTOR");
                ...
                if(!enabled_ar_detect.waitForExistence())
                {
                    ROS_WARN("Waiting for the /aruco_detect server to come up, please check /aruco_detect node is available");
                }
                else
                {
                    setDetect_AR("enable");
            ...
            case ARDockingControlState::GOAL_CHECKER:
                ROS_INFO("[Matrix_docking_ar_operation]: GOAL_CHECKER");
                // setDockingMode("on");
            ...
            {
                ROS_INFO("[Matrix_docking_ar_operation]: SEND_CG_END"); 

                data_send = "CHARGE_FINISH" + cg_id_str;
                data_receive = "CG_READY" + cg_id_str;
            ...
            case ARDockingControlState::WAIT_CG_FINISH:
                ...
                {   
                    ...
                    {
                        results_.result = ARDockingOperationResult::RESULT_DOCKING_NOT_RESPONSE;
                ...
                else
                {
                    ...
                    {
                        ROS_INFO("[matrix_docking_ar_operation]: Check timeout");
                        feedback_.timeout = setTimeOut - ros::Time::now().toSec();
                        ROS_INFO("[matrix_docking_ar_operation]: Check timeout countdown %d", feedback_.timeout);
                        //check time out
                        if(ros::Time::now().toSec() > setTimeOut)
                        {
                            results_.success = false;
                            results_.result = ARDockingOperationResult::RESULT_CONTROL_SEQUENCE_TIMEOUT;
            ...
            {
                ROS_INFO("[Matrix_docking_ar_operation]: SEND_CG_CHECK");
                ...
                rs485_goal.arg_sr_send = "#" + CHARGE_CHECK + cg_id_str + "$";
                rs485_goal.arg_strings.push_back(CHARGE_OK + cg_id_str);
                rs485_goal.arg_strings.push_back(CHARGE_NOT_OK + cg_id_str);
            ...
            case ARDockingControlState::WAIT_CG_OK:
                ROS_INFO("[Matrix_docking_ar_operation]: WAIT_CG_OK");
                feedback_.text = "communication with docking in WAIT_CG_OK, wait -->" + (CHARGE_OK + cg_id_str) + ", " + (CHARGE_NOT_OK + cg_id_str);
                ...
                {   
                    ...
                    if(serial_results->result == SerialOperationResult::SUCCESS)
                    {
                        if(serial_results->data_bypass == (CHARGE_OK + cg_id_str))
                        {
                            feedback_.sequence = ARDockingControlState::SEARCH_AR;
                        }
                        else
                        {
                            ...
                            results_.result = ARDockingOperationResult::RESULT_DOCKING_NOT_OK;
                            ROS_ERROR("[Matrix_docking_ar_operation]: %s", serial_results->data_bypass.c_str());
                            feedback_.sequence = ARDockingControlState::ERROR;
                        }
                    ...
                    else
                    {
                        ...
                        results_.sequence = ARDockingControlState::WAIT_CG_OK;
                        ROS_ERROR("[Matrix_docking_ar_operation]: %s", results_.text.c_str());
                        feedback_.sequence = ARDockingControlState::ERROR;
                    }
                }
                else
            ...
            case ARDockingControlState::SEARCH_AR:
                ROS_INFO("[Matrix_docking_ar_operation]: SEARCH_AR");
                if(setDockingMode("on"))
                {
                    feedback_.text = "SEARCH_AR";
            ...
            case ARDockingControlState::INIT_AR_TRACKING:
                ROS_INFO("[Matrix_docking_ar_operation]: INIT_AR_TRACKING");

                // set Serial action rs485 for wait data --> #ROBOT_CONNECTED_CG + CG_ID$
                data_send = "";
                data_receive = "WAIT_ROBOT_REQ_CG" + cg_id_str;
            ...
            case ARDockingControlState::AR_TRACKING:
            {
                ...
                else
                {
                    ...
                    setTimeOut = ros::Time::now().toSec() + goal->ar_tracking_timeout; 
                    feedback_.sequence = ARDockingControlState::WAIT_FINISH_AR_TRACKING;
                }
            ...
            case ARDockingControlState::WAIT_FINISH_AR_TRACKING:
                ROS_INFO("[Matrix_docking_ar_operation]: WAIT_FINISH_AR_TRACKING");
                ...
                else
                {
                    if(Matrix_ARTrack3Completed)
                    {
                        ...
                        {
                            results_.result = ARDockingOperationResult::RESULT_FAIL_AR_TRACK;
                            results_.success = false;
                            results_.text = "fail " + feedback_.text;
                            results_.sequence = feedback_.sequence;
                            ROS_ERROR("[Matrix_docking_ar_operation]: %s", results_.text.c_str());
            ...
            case ARDockingControlState::WAIT_CG_CONFIRM_CONNECT:
                ROS_INFO("[Matrix_docking_ar_operation]: WAIT_CG_CONFIRM_CONNECT");
            ...
            {
                ROS_INFO("[Matrix_docking_ar_operation]: SEND_CG_REQ_ON");
                ...
                data_send = CHARGE_REQ + cg_id_str;
                data_receive = CG_REQ_SUCCESS + cg_id_str;
                goal_timeout = goal->communication_timeout;
            ...
            case ARDockingControlState::WAIT_CG_REQ_ON_SUCCESS:
                ROS_INFO("[Matrix_docking_ar_operation]: WAIT_CG_REQ_ON_SUCCESS");
            ...
            case ARDockingControlState::INIT_BMS_CONFIRM_STATE:
                ROS_INFO("[Matrix_docking_ar_operation]: INIT_BMS_CONFIRM_STATE");
            ...
            case ARDockingControlState::WAIT_BMS_CONFIRM_STATE:
                ROS_INFO("[Matrix_docking_ar_operation]: WAIT_BMS_CONFIRM_STATE");
            ...
            case ARDockingControlState::SEND_CONFIRM_STATE_TO_DOCK:
                ROS_INFO("[Matrix_docking_ar_operation]: SEND_CONFIRM_STATE_TO_DOCK");
                feedback_.text = "SEND_CONFIRM_STATE_TO_DOCK";
                // set Serial action rs485 for send data for request charger on --> #ROBOT_CONNECTED_CG + CG_ID$
                data_send = ROBOT_CHARGING + cg_id_str;
                data_receive = CG_CHARGING_STATE + cg_id_str;
                goal_timeout = goal->communication_timeout;
                setTimeOut = ros::Time::now().toSec() + goal_timeout;
                SerialActionSend(SerialOperationGoal::send_receive, 
                                    data_send, 
            ...
            case ARDockingControlState::WAIT_CG_CONFIRM_STATE:
                ROS_INFO("[Matrix_docking_ar_operation]: WAIT_CG_CONFIRM_STATE");
            ...
            {   ROS_INFO("[Matrix_docking_ar_operation]: INIT_CG_CONDITION_FINISH");
            ...
            case ARDockingControlState::CHECK_CONDITION_FINISH:
                ROS_INFO("[Matrix_docking_ar_operation]: CHECK_CONDITION_FINISH");
                feedback_.text = "CHECK_CONDITION_FINISH";

                //if charging_time set 0 --> ignore charging_time
                if(goal->charging_time != 0)
                {
                    ROS_INFO("[matrix_docking_ar_operation]: Check charging_time");
                    feedback_.timeout = charging_time - ros::Time::now().toSec();
                    ROS_INFO("[matrix_docking_ar_operation]: Check charging_time countdown %d", feedback_.timeout);
                    //check time out
                    if(feedback_.timeout <= 0)
                    {
                        results_.success = true;
                        results_.result = ARDockingOperationResult::RESULT_TIMMER_FINISH;
                ...
                {
                    results_.success = true;
                    results_.result = ARDockingOperationResult::RESULT_BMS_FINISH;
                ...
                {
                    results_.success = true;
                    results_.result = ARDockingOperationResult::RESULT_SERIAL_FINISH;
                ...
                if(results_.success)
                {
                    ...
                    // rs485_goal.arg_string = "#CHARGE_FINISH4$";
                    // Matrix_SerialCompleted = false;
            ...
            case ARDockingControlState::DODCKING_NOT_SYNC:
                ROS_INFO("[Matrix_docking_ar_operation]: ERROR");
            ...
            case ARDockingControlState::PAUSE:
                ROS_INFO("[Matrix_docking_ar_operation]: PAUSE");
            ...
            case ARDockingControlState::FINISH:
                ROS_INFO("[Matrix_docking_ar_operation]: FINISH");
...
void ARDockingOperation::clear_params(bool cancel_action_ar3, bool kill_action_ar3)
{
    ...
    artrack3_operation_ac.cancelAllGoals();
    setDetect_AR("disable");
    // std_srvs::SetBool srv;
    // srv.request.data = false;
    // if(enabled_ar_detect.call(srv))
    // {
    //     ROS_WARN("[Matrix_docking_ar_operation]: AR3 Detection state:%d", srv.request.data);
    // }
...
bool ARDockingOperation::setDetect_AR(std::string cmd)
{
    std_srvs::SetBool srv;
    srv.request.data = (cmd == "enable")? true:false;
    enabled_ar_detect.call(srv);
    ROS_WARN("[Matrix_docking_ar_operation]: AR3 Detection state:%d", srv.request.data);
...
void ARDockingOperation::cbRS485Receive(std_msgs::String::ConstPtr msg)
{
    ...
    if((current_sequence >= ARDockingControlState::SEARCH_AR) && (current_sequence <= ARDockingControlState::WAIT_CG_CONFIRM_STATE))
    {
        ...
        if(_isDockingStateSync)
        {
           ...
           ROS_INFO("1111");
            if(a.find("ROBOT_DISCONNECT_DOCKING") != -1)
            {
...
{
    ros::init(argc, argv, "matrix_docking_ar_operation");

    ARDockingOperation DockingOperation("matrix_docking_ar_operation");
    ros::spin();
