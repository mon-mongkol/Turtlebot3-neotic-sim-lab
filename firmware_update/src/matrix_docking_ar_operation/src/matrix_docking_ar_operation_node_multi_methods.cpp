    ...
    while(!finished)
    {
        ...
        switch(feedback_.sequence)
        {
            ...
            case ARDockingControlState::CMD_SELECTOR:
                ...
                if((goal->method == "cvshape_method") || (goal->method == "ar3track_method"))
                {
                    feedback_.sequence = ARDockingControlState::GOAL_CHECKER;
                }
                else if((goal->method == "get_docking_id") || (goal->method == "acknow_docking"))
                {
                    feedback_.sequence = ARDockingControlState::DOCKING_MANAGEMENT;
                }
                else
                {
                    ROS_ERROR("[matrix_docking_multi_methods_operation]: %s method not found!", goal->method.c_str());
                }
            ...
            case ARDockingControlState::DOCKING_MANAGEMENT:
                ROS_INFO("[matrix_docking_multi_methods_operation]: DOCKING_MANAGEMENT");
                if(goal->method == "get_docking_id")
                {
                    SerialActionFindKey("DG_GET_ID", "RxID", 30, false);
                    setTimeOut = ros::Time::now().toSec() + goal_timeout + 10;
                    feedback_.sequence = ARDockingControlState::DOCKING_MANAGEMENT_RES;
                }
                else if(goal->method == "acknow_docking")
                {
                    SerialActionFindKey("DG_ACK_"+goal->charger_id_str, "RxID"+goal->charger_id_str, 30, true);
                    setTimeOut = ros::Time::now().toSec() + goal_timeout + 10;
                    feedback_.sequence = ARDockingControlState::DOCKING_MANAGEMENT_RES;
                }
                else
                {
                    ROS_ERROR("[matrix_docking_multi_methods_operation]: DOCKING_MANAGEMENT %s method not found!", goal->method.c_str());
                }
                break;
            
            case ARDockingControlState::DOCKING_MANAGEMENT_RES:
                ROS_INFO("[matrix_docking_multi_methods_operation]: DOCKING_MANAGEMENT_RES");
                ...
                {   
                    ...
                    if(serial_results->result == SerialOperationResult::SUCCESS)
                    {
                        ...
                        if(goal->method == "get_docking_id")
                        {
                            results_.result = ARDockingOperationMultiMethodsResult::RESULT_GET_ID_SUCCESS;
                            std::string s1 = serial_results->data_bypass;
                            std::string docking_id_ = s1.substr(5, 16);
                            results_.text = docking_id_;
                        }
                        else if(goal->method == "acknow_docking")
                        {
                            results_.result = ARDockingOperationMultiMethodsResult::RESULT_FOUND_DOCKING;
                            std::string s1 = serial_results->data_bypass;
                            std::string docking_id_ = s1.substr(5, 16);
                            results_.text = docking_id_;
                        }else;
                    ...
                    else
                    {
                        ...
                        results_.text = "fail communication with docking in DOCKING_MANAGEMENT_RES, wait -->" + data_receive;
                        results_.sequence = ARDockingControlState::DOCKING_MANAGEMENT_RES;
            ...
            case ARDockingControlState::INIT_AR_TRACKING:
                ...
                data_send = "INIT_AR_TRACKING" + DG_SN;
            ...
            case ARDockingControlState::AR_TRACKING:
            {
                ...
                else
                {
                    ...
                    // set autodock stop distance
                    if(DG_SN.find("DARA") != -1)
                    {
                        stop_distance = ar3_docking_DARA;
                    }
                    else if(DG_SN.find("DARB") != -1)
                    {   
                        stop_distance = ar3_docking_DARB;
                    }
                    else
                    {
                        ;
                    ...
                    dynamic_reconfigure::DoubleParameter stop_distance_;
                    stop_distance_.name = "stop_distance";
                    stop_distance_.value = stop_distance;
                    srv.request.config.doubles.push_back(stop_distance_);
                    autodock_sc.call(srv);


                    // call action server /autodock [ar3 tracking]
                    autodock_core::AutoDockingGoal artrack3_goal;
                    artrack3_goal.use_cfg_yaml = true;
                    artrack3_goal.use_stop_from_dynamics = true;
            ...
            case ARDockingControlState::CV_SHPAE_TRACKING:
                ...
                else
                {
                    ...
                    // different stop range special for job J402B0002 (Korat)
                    cv_goal_yaml.stop_dist = (goal->charger_id_str.find("DARB") == -1)?  cv_default_stop_dist:cv_default_stop_dist_B; 
...
void ARDockingOperation::clear_params(bool cancel_action_ar3, bool kill_action_ar3)
{
    ...
    cvshape_operation_ac.cancelAllGoals();
...
{  
    ...
    if(motor_model != "matrix_navgo_controller") 
    {
        dynamic_reconfigure::Reconfigure srv;
        if(cmd == "Unlock_Motor")
        {
            ...
            ROS_WARN("unlock_motor.value = %d", unlock_motor.value);
    ...
    {
        ; //break not available in matrix_navgo_controller 
    }
...
void ARDockingOperation::SerialActionFindKey(std::string send, std::string receive, double timeout, bool repeat)
{
    SerialOperationGoal rs485_goal;
    rs485_goal.cmd = "find_data_in_payload";
    rs485_goal.arg_sr_send = "#" + send + "$";
    rs485_goal.arg_strings.push_back(receive);
    rs485_goal.timeout = abs(timeout);
...
// void ARDockingOperation::SerialActionFindDatapayload()
// {
//     matrix_msgs::SerialOperationActionGoal a;
// }

void ARDockingOperation::dynamics_callback(matrix_docking_ar_operation::DockingARConfig &config, uint32_t level)
{
    ar3_distance_from_dynamics = config.ar3_distance_from_dynamics;
    ar3_docking_DARA = config.ar3_docking_DARA;
    ar3_docking_DARB = config.ar3_docking_DARB;
}
...
void ARDockingOperation::cbRS485Receive(std_msgs::String::ConstPtr msg)
{
    ...
    if((current_sequence > ARDockingControlState::SEARCH_AR) && (current_sequence <= ARDockingControlState::WAIT_CG_CONFIRM_STATE))
    {
        ...
        if(_isDockingStateSync)
        {
           ...
           ROS_INFO("1111");
            if((a.find("RxRBDCN") != -1) && (a.find(DG_SN) != -1))
            {
                _isDockingStateSync = false;
                docking_state_str=msg->data;
            }
            else if((a.find("PLS_PRESS_BUSBAR") != -1) && (a.find(DG_SN) != -1))
            {
                _isDockingStateSync = false;
                docking_state_str=msg->data;
            }
            else if((a.find("CHARGE_NOT_OK") != -1) && (a.find(DG_SN) != -1))
            {
                _isDockingStateSync = false;
                docking_state_str=msg->data;
            }
            else if((a.find("RxEND") != -1) && (a.find(DG_SN) != -1))
            {
                _isDockingStateSync = false;
                docking_state_str=msg->data;
            }
...
void ARDockingOperation::setBusbar(std::string mode)
{
    if(serial_no.find("2024") != -1)
    {
        std_msgs::Bool data_msg;
        data_msg.data = (mode == "enable")? true:false;
        busbar_pub.publish(data_msg);
    }
    else
    {
        matrix_msgs::SetIOs set_ios_srv;
        set_ios_srv.request.cmd = "enable_busbar";
        set_ios_srv.request.arg0 = (mode == "enable")? 1:0;
        set_ios_srv.request.arg1 = 0;
        set_ios_sc.call(set_ios_srv);
    }
}
