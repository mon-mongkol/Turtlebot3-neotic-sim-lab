...
bool DockingOperation::executeCB(const matrix_msgs::ROHMChargerOperationGoalConstPtr &goal)
{
        ...
        switch(feedback_.sequence)
        {
            ...
            case DockingControlState::CmdSelector:
                if(goal->cmd == matrix_msgs::ROHMChargerOperationGoal::check_charger_end)
                {
                    ...
                    {
                        EnableBusbar("enable");
                        setTimeOut = ros::Time::now().toSec() + charging_state_timeout + 10;
                    }
                    else{
    ...
    {
        results_.result = matrix_msgs::ROHMChargerOperationResult::PREEMPT;
        ROS_WARN("[Matrix_docking_operation]: SetResult Pree,pt");
    }
    else
    {
...
// }
void DockingOperation::EnableBusbar(std::string enable)
{
    // matrix_msgs::SetIOs set_ios_srv;
    // set_ios_srv.request.cmd = "enable_busbar";
    // set_ios_srv.request.arg0 = (enable == "enable")? 1:0;
    // set_ios_srv.request.arg1 = 0;
    // set_ios_sc.call(set_ios_srv);

    std_msgs::Bool msg;
    msg.data = (enable == "enable")? true:false;
    if(msg.data)
    {
        _BusbarOn = true;
    }
...
{
    ROS_WARN("%s got preempteddddddddd!", action_name_.c_str());
    finished = true;
    success = false;
    clear_params();
    fnStop();
    ...
    results_.result = matrix_msgs::ROHMChargerOperationResult::PREEMPT;
    results_.text = "got preempted and working with new goal!";
    as_.setSucceeded(results_);
    as_.setPreempted();
}
...
void DockingOperation::cbIOInput(std_msgs::Int32MultiArray msg)
{
    if(!_isAutocharge)
    {
        if(msg.data[8] == 0)
        {
            if((msg.data[2] == 0) && (msg.data[1] == 0))
            {
                EnableBusbar("enable");
                setDockingMode("on");
            }else;
