...
class DockingOperation
{
    protected:
        ...
        ros::ServiceClient docking_mode_sc, set_ios_sc;
        ros::Subscriber serial_sub, batt_sub, odom_sub, robotmode_sub, ioinput_sub;
        ros::Publisher cmd_vel_pub, busbar_pub;
        ...
        bool enable_busbar = true;
        bool _isAutocharge = false;
        bool _BusbarOn = false;
    ...
    public:
...

        {
            ...
            ros::param::param<bool>("~enable_busbar", enable_busbar, true);
            // ros::param::param<bool>("~enable_busbar", enable_busbar, true);
            ...
            ioinput_sub = nh_.subscribe("/matrix_io/input", 1, &DockingOperation::cbIOInput, this);
            ...
            ROS_INFO("matrix_docking_operation action server is readyyyy!");
        }
    ...
    private:
        ...
        void cbIOInput(std_msgs::Int32MultiArray msg);
        ...
        void EnableBusbar(std::string enable);
