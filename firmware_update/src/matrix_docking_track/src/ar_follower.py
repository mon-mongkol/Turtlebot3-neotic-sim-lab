...
class ARFollower():
    def __init__(self):
        ...
        self.max_angular_speed = rospy.get_param("~max_angular_speed", 0.25) #defaut 2.0
        
        # The minimum rotation speed in radians per second
        self.min_angular_speed = rospy.get_param("~min_angular_speed", 0.07) #defaut 0.5
        
        # The maximum distance a target can be from the robot for us to track
        self.max_x = rospy.get_param("~max_x", 4.0) # defaut 20
        
        # The goal distance (in meters) to keep between the robot and the marker
        self.goal_x = rospy.get_param("~goal_x", 0.98) ######## changed

        #self.goal_x = rospy.get_param("~goal_x", 1.9)
	
        # The goal distance (in meters) to keep between the robot and the marker
        self.goal_y = rospy.get_param("~goal_y", 0.13) ######## changed
        ...
        self.y_threshold = rospy.get_param("~y_threshold", 0.03) ######## changed
