...
#include "matrix_msgs/DistanceAngle.h"
#define flags 1
#define PI 3.14
...
matrix_msgs::DistanceAngle marker;
//float chassis {0.05};
...
public:
    ...
    void ComputeRPY(const tf2_msgs::TFMessage& ar_marker, int i) {  /* from quaternion to euler angles */
     ...
     void CopytoMarker(matrix_msgs::DistanceAngle &marker){
...

{
  ...
  if ((robot.transforms[0].header.frame_id == "depth_cam_mount_point_color_optical_frame")&&(robot.transforms[0].child_frame_id == "ar_marker_4")) //I should define the frames above!
 {
...
{ 
  ...
  DistanceAngle_pub = n.advertise<matrix_msgs::DistanceAngle>("DistanceAngle", 1000);
