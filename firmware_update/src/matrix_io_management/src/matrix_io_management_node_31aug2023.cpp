...
class ManagementIO {
    ...
    private:
        ros::NodeHandle nh_;
        int counter;
        ros::Publisher pub_raw_read, pub_bumper_state, pub_usr_pin_x, pub_usr_pin_y;
        ...
        int output_state_prev[4] = {0,0,0,0};
        int y_cfg[4][4];
        std_msgs::Int32MultiArray output_state;
        int output[4];

        bool wait_data_comeup = false;
        // std_msgs::Int32MultiArrayConstPtr iostate_;
    ...
    ManagementIO() {
        ...
        // iostate_ = ros::topic::waitForMessage<std_msgs::Int32MultiArray>("/matrix_io/output", ros::Duration(10));

        ros::param::param<int>("~y1_int", y_cfg[0][1], 0);
        ros::param::param<int>("~y2_int", y_cfg[1][1], 0);
        ros::param::param<int>("~y3_int", y_cfg[2][1], 0);
        ros::param::param<int>("~y4_int", y_cfg[3][1], 0);

        // y_cfg[0][0] = iostate_->data[10];
        // y_cfg[1][0] = iostate_->data[11];
        // y_cfg[2][0] = iostate_->data[12];
        // y_cfg[3][0] = iostate_->data[13];
    ...
    bool scan_time_control_intCase()
    {
        ...
        for(int i= 0; i<4; i++)
        {
            ...
            // check enabled
            // if(y_cfg[i][0])
            if(y_cfg[i][0] != 2) // 0 = off, 1 = on, 2 = disabled
            {
                // ROS_INFO("AA");
                try{
                    if(y_cfg[i][1] != output_state.data[10+i])
                    {
                        srv_setios.request.arg0 = i+1;
                        srv_setios.request.arg1 = y_cfg[i][1];
                        // ROS_INFO("%d", y_cfg[i][1]);
                        ROS_INFO("[matrix_io_management]: action at user_output_chanel:%d, with sate: %d", i+1, srv_setios.request.arg1);
                        try
                        {
                            if(set_io_sc.call(srv_setios))
        ...
        // {
            ...
            // config.y4_on = (config.y4_enabled)? y_cfg[3][2]:false;
                ...
                // {
                    ...
                    {
                        y_cfg[0][0] = output_state.data[10];
                        y_cfg[0][1] = config.y1_int;
                        ROS_INFO("config.y1_int:%d", config.y1_int);
                    }
        

        
                    if(output_state.data[11] == 2)
                    {
                        y_cfg[1][0] = 2;
                        y_cfg[1][1] = 2;
                    }
                    ...
                    {
                        y_cfg[1][0] = output_state.data[11];
                        y_cfg[1][1] = config.y2_int;
                    }
                

                    
                    if(output_state.data[12] == 2)
                    {
                        y_cfg[2][0] = 2;
                        y_cfg[2][1] = 2;
                    }
                    ...
                    {
                        y_cfg[2][0] = output_state.data[12];
                        y_cfg[2][1] = config.y3_int;
                    }

                
                    if(output_state.data[13] == 2)
                    {
                        y_cfg[3][0] = 2;
                        y_cfg[3][1] = 2;
                    }
                    ...
                    {
                        y_cfg[3][0] = output_state.data[12];
                        y_cfg[3][1] = config.y4_int;
                    }

                    for(int i =0; i<4; i++)
                    {
                        ROS_INFO("*");
                        ROS_INFO("y_cfg[%d][0]: %d",  i, y_cfg[i][0]);
                        ROS_INFO("y_cfg[%d][1]: %d",  i, y_cfg[i][1]);
                        ROS_INFO("*");
                    }

                    scan_time_control_intCase();

                    config.y1_int = (output_state.data[10] != 2)? y_cfg[0][2]:2;
                    config.y2_int = (output_state.data[11] != 2)? y_cfg[1][2]:2;
                    config.y3_int = (output_state.data[12] != 2)? y_cfg[2][2]:2;
                    config.y4_int = (output_state.data[13] != 2)? y_cfg[3][2]:2;

                    // ROS_INFO("config.y1_int %d, %d", config.y1_int, output_state.data[10]);
                    // ROS_INFO("config.y2_int %d, %d", config.y2_int, output_state.data[11]);
                // }
    ...
    void cbMCUOutput(const std_msgs::Int32MultiArray msg)
    {
        ...
        // {   
        //     ROS_INFO("ComeUpp");
        //     matrix_io_management::IOManagementConfig config;
        //     uint32_t level;
