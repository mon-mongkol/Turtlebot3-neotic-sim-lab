#include "matrix_shelf_lift_operation/matrix_shelf_lift_operation_2024March20.h"
    ...
    while(!finished)
    {
        ...
        switch(feedback_.sequence)
        {
            ...
            case SHLControlState::CHECK_LIFT_STATE:
                ...
                STRcondition_checking("IDL", "BUSY", "emergency", "", "check");
            ...
            case SHLControlState::WAIT_CHECK_LIFT_STATE_FINISH:
            {
                ...
                else if(SHLState.find("emergency") != -1)
                {
                    results_.result = SHLOperationResult::emer_active;
                    results_.text = "control sequence timeout -->" + feedback_.text + "| DATA--> " + SHLState;
                    results_.sequence = feedback_.sequence;
                    feedback_.sequence = SHLControlState::ERROR;
                    ROS_WARN("%s", results_.text.c_str());
            ...
            case SHLControlState::SEND_CMD:
            {
                ...
                if(goal->cmd != "rst")
                {
                    ...
                    STRcondition_checking("AcResult", "AcResult", "AcResult", "AcResult", "check");
                }
                else
                {
                    // STRcondition_checking("INIT_Success", "INIT_FINISH", "check");

                    STRcondition_checking("INIT_FINISH", "INIT_FINISH", "INIT_FINISH", "INIT_FINISH", "check");
                } 
            ...
            case SHLControlState::WAIT_RES_CMD:
                ...
                // Command respond match with goal 
                if((data_receive.find(cmd_msg.data) != -1) || (data_receive.find('BUSY_AcCONTROL') != -1) || (data_receive.find('AcRecvGoal') != -1))
                {
                    // set time_out when timeout != 0, and set defaul 20s when timeout == 0 
                    // if(abs(goal->timeout) != 0)
                    // {
                    //     setTimeOut = ros::Time::now().toSec() + abs(goal->timeout);
                    // }
                    // else
                    // {
                    ...
                    if((goal->timeout) == -1)
                    {
                        setTimeOut = ros::Time::now().toSec() + 30;
                    }
                    else if(abs(goal->timeout) == 0)
                    {
                        setTimeOut = 0;
                    }
                    else
            ...
            case SHLControlState::WAIT_RESULT:
            {
                ...
                std::string SHLResult = STRcondition_checking("", "", "", "", "match_state");
                // ROS_WARN("Result of STRcondition_checking is --> %s", SHLResult.c_str());
                if((SHLResult.find("Result") != -1) && (goal->cmd != "rst"))
                {   
                    int result;
                    if(SHLResult.find("Result[10]$") != -1)
                    {
               
                        result = 10;
                    }else if((SHLResult.find("Result[11]$") != -1))
                    {
                        result = 11;
                    }else
                    {
                        ...
                        result = SHLResult[SHLResult.length()-3] - '0';
                ...
                else
                {
                    if(goal->cmd == "rst")
                    {
                        ROS_INFO("RESET CHECK RESULT");
                        // if(SHLResult == "#RxSHL100_[INIT_FINISH]$")
                        if(SHLResult.find("[INIT_FINISH]") != -1)
                        {
                    ...
                    //ignore when abs(goal->time_out) == 0
                    if(abs(goal->timeout) != 0)
                    {
                        if(ros::Time::now().toSec() > setTimeOut)
                        {
        ...
        //     pub_shl100_write.publish(cmd_stop);
        // }
        if(((current_robotmode == matrix_msgs::RobotMode::EMERGENCY) || (isLiftEmerActive)) && (feedback_.sequence != SHLControlState::ERROR))
        {
            ;
            // ROS_WARN("[matrix_shelf_lift_operation]: %s Emergency state setAborted!", action_name_.c_str());
            // _isAborted = true;

            // //set lift stop moving 
            // std_msgs::String cmd_stop;
            // cmd_stop.data = "#Tx"+lift_model+"_STOP$";
            // pub_shl100_write.publish(cmd_stop);

            // results_.result = SHLOperationResult::emer_active;
            // results_.text = "Base Emergency or Lift Emergency Active!!!"; 
            // results_.sequence = feedback_.sequence;
            // feedback_.sequence = SHLControlState::ERROR;
            // ROS_WARN("%s", results_.text.c_str());
...
std::string SHLOperation::STRcondition_checking(std::string con1, std::string con2, std::string con3, std::string con4, std::string cmd)
{
    std::string data_bypass = "";
    if(cmd == "check")
    {   
...
void SHLOperation::cbSHLReceive(std_msgs::String::ConstPtr msg)
{
    ...
    ROS_WARN("Receive form Lifter --> %s, condition --> %s, %s, %s, %s", msg->data.c_str(), condition1.c_str(), condition2.c_str(), condition3.c_str(), condition4.c_str());
    if((condition1 != "") || (condition2 != ""))
    {

        if(!isConditionMatch )
        {
            ROS_INFO("condition checking.... %s", condition1.c_str());
            if((msg->data.find(condition1) != -1) || (msg->data.find(condition2) != -1 ))
            {
                conditionStr = msg->data;
                isConditionMatch = true;
                ROS_ERROR("condition match.... 12  %s", conditionStr.c_str());
            }
        }
    ...
    if((condition3 != "") || (condition4 != ""))
    {
        if(!isConditionMatch )
        {
            ROS_WARN("condition checking.... 34  ");
            if((msg->data.find(condition3) != -1) || (msg->data.find(condition4) != -1 ))
            {
                conditionStr = msg->data;
                ROS_ERROR("condition match.... 34  %s", conditionStr.c_str());
                isConditionMatch = true;
            }
        }
    ...
    if(msg->data.find("emergency") != -1)
    {
        ROS_WARN("[matrix_shelf_lift_operation]: Lift in Emergency state!!!");
        isLiftEmerActive = true;
    }
    else
    {
        isLiftEmerActive = false;
    }
}
