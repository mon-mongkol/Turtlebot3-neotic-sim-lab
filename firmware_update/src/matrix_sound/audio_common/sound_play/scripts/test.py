...
from sound_play.cfg import SoundPlayConfig
...
class sound_play:
    def __init__(self):
        ...
        self.robot_mode = rospy.Subscriber("/matrix_mode_controller/mode", RobotMode, self.cbRobotMode)
        # self.sub_scan = rospy.Subscriber("/scan", LaserScan, self.cbScan)
        self.sub_odom = rospy.Subscriber("/odom", Odometry, self.cbOdom)
        self.sub_robot_status = rospy.Subscriber("/matrix_system/movement_status", MovmentStatus, self.cbRobotState)
        # self.sound_mode = rospy.Service("/sound_mode", SoundMode, self.cbSound)

        self.srv_sound_play = Server(SoundPlayConfig, self.cbGetSoundPlayParam)
        ...
        self.robot_state = MovmentStatus()
        self.counter = 0
        ...
        self.enabled_movement_sound = True
        self.enabled_button_sound = True
    ...
    def cbGetSoundPlayParam(self, config, level):
        rospy.loginfo("[matrix_sound_play] sound play Parameter reconfigured to")
        rospy.loginfo("enabled_movement_sound: %d", config.enabled_movement_sound)
        rospy.loginfo("enabled_button_sound: %d", config.enabled_button_sound)
        self.enabled_movement_sound = config.enabled_movement_sound
        self.enabled_button_sound = config.enabled_button_sound
        return config


    def cbRobotState(self, msg):
        self.robot_state.robot_moving = msg.robot_moving
    ...
    def cbRobotMode(self, msg):
        ...
        if self.current_robot_mode != msg.robot_mode or self._isInit == False:
            ...
            if self.enabled_button_sound:
                ...
                elif msg.robot_mode == self.Robot_Mode.TABLET_LOSS_COMMU.value:
                    pass
                    # self.soundhandle.say('Tablet disconnected')
                    # self.soundhandle.playWave('Tablet disconnected.mp3')
                elif msg.robot_mode == self.Robot_Mode.DOCKING_MODE_ON.value:
                    pass
            ...
            if (msg.robot_mode == 2) or (msg.robot_mode == self.Robot_Mode.UVC_OFF.value) or (msg.robot_mode ==self.Robot_Mode.DOCKING_MODE_OFF.value):
                self._isMoving = True
                self._isBackward = False
            elif msg.robot_mode == self.Robot_Mode.DOCKING_MODE_ON.value:
                self._isMoving = False
                self._isBackward = True
            elif msg.robot_mode == self.Robot_Mode.EMERGENCY.value:
                self._isMoving = False
                self._isBackward = False
            else:
                self._isMoving = False
        ...
        # # print(msg.robot_mode, self._isMoving, self._isBackward)
