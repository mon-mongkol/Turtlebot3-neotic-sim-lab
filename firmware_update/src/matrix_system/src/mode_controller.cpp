...
private:
    ...
    bool _isAnotherAutocharge = false;
...
public:
    ...
    void check_anotherAutocharge()
    {
        _isAnotherAutocharge = false;
        if(_isCharging)
        {
            //check robot isn't in RobotMode::EMERGENCY_CHARGE
            if (!_isMCUEmergency_charge || !_isSOFTWAREEmergency_charge)
            {
                //check robot isn't in charging docking methods
                if(!_isDocking_mode)
                {
                    _isAnotherAutocharge = true;
                }
            }
        }
    }
    ...
    void mode_manager3()
    {
        ...
        if (_isSystemReady == Diagnostics::SYSTEM_READY_STATUS_OK)
        {
            ...
            if (!_isEmerCharge_state)
            {
                ...
                else
                {
                    clear_emer_state();
                    // ROS_INFO("RobotMode::EMERGENCY");
                    mode_msg.robot_mode = RobotMode::CHARGER_ON;
                    _isInit_state = false;
                }
            }
            else
        ...
        ROS_INFO("[matrix_mode_contrller]: current robot mode %d", mode_msg.robot_mode);
