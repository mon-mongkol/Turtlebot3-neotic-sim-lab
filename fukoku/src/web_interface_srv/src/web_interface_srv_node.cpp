...
enum StateEnum
{
  ...
  FINISH_PROCESS,
  STOP_MOVE_BASE,
  WAIT_MOVE_BASE_PROCESS,
  START_MOVE_BASE,
  ADAPTER_START_PROCESS
};
...
private:
  ...
  bool newMap(std::vector<std::string> args)
  {
    ...
    while (!finished)
    {
      ...
      case StateEnum::STOP_PROCESS:
        if (launchCompleted)
        {
          if (launchClient.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
          {
            ...
            state = StateEnum::STOP_MOVE_BASE;
          }
          if (launchClient.getState() == actionlib::SimpleClientGoalState::ABORTED)
          {
            return false;
      ...
      //**************************************************//
      //**************************************************//
      //********Maintainer Jarubank 21 Oct 2023 13:12 ****///
      //**************************************************//
      //**************************************************//
      case StateEnum::STOP_MOVE_BASE:
      {
        ROS_INFO("Reset ODOM");
        ros::ServiceClient ekf_ = nh_.serviceClient<robot_localization::SetPose>("set_pose");
        robot_localization::SetPose reset_srv;
        ekf_.call(reset_srv);


        ROS_INFO("STOP_MOVE_BASE");
        goal.command = "stop";
        goal.args.clear();
        goal.args.push_back("move_base");


        launchCompleted = false;
        launchClient.sendGoal(goal, boost::bind(&WebAction::launchDoneCb, this, _1), WebLaunchClient::SimpleActiveCallback());
        feedback_.sequence.push_back("stop move_base");
        as_.publishFeedback(feedback_);
        state = StateEnum::WAIT_MOVE_BASE_PROCESS;
        ROS_INFO("sendGoal stop move_base");
        break;
      }
      
      case StateEnum::WAIT_MOVE_BASE_PROCESS:
        if (launchCompleted)
        {
          if (launchClient.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
          {
            ROS_INFO("stop move_base SUCCEEDED");
            feedback_.sequence.push_back("stop move_base SUCCEEDED");
            as_.publishFeedback(feedback_);
            _isMovebaseRunning = false;
    ...
    while (!finished)
    {
      ...
      case StateEnum::START_PROCESS:
        if (launchCompleted)
        {
          if (launchClient.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
          {
            ...
            //   state = StateEnum::FINISH_PROCESS;
            // }
            state = StateEnum::START_MOVE_BASE;
          }
          if (launchClient.getState() == actionlib::SimpleClientGoalState::ABORTED)
          {
            return false;
        ...
        break;
      case StateEnum::START_MOVE_BASE:
        ROS_INFO("START_MOVE_BASE");
        if(!_isMovebaseRunning)
        { 
          goal.command = "start";
          goal.args.clear();
          goal.args.push_back("move_base");
          ...
          launchCompleted = false;
          launchClient.sendGoal(goal, boost::bind(&WebAction::launchDoneCb, this, _1), WebLaunchClient::SimpleActiveCallback());
          feedback_.sequence.push_back("start move_base");
          
          state = StateEnum::WAIT_MOVE_BASE_PROCESS;
          ROS_INFO("sendGoal start move_base");
        }
        else
        {
          state = StateEnum::ADAPTER_START_PROCESS;
          ROS_INFO("move_base is running");
          feedback_.sequence.push_back("move_base is running");
        }
        as_.publishFeedback(feedback_);
      ...
      case StateEnum::WAIT_MOVE_BASE_PROCESS:
        if (launchCompleted)
        {
          if (launchClient.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
          {
            ROS_INFO("start move_base SUCCEEDED");
            feedback_.sequence.push_back("start move_base SUCCEEDED");
            as_.publishFeedback(feedback_);
            _isMovebaseRunning = true;
            state = StateEnum::ADAPTER_START_PROCESS;
          }
          if (launchClient.getState() == actionlib::SimpleClientGoalState::ABORTED)
          {
      ...
      case StateEnum::ADAPTER_START_PROCESS:
        feedback_.sequence.push_back("check robot pose ADAPTER_START_PROCESS");
        as_.publishFeedback(feedback_);
        if (ck_tr)
        {
          //set robot position
